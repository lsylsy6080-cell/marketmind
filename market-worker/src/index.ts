...
async function main(): Promise<void> {
  console.log(
    "BTCUSDT 1분봉 수집을 시작합니다.",
  );

  const savedCount =
    await collectBinanceBtcCandles(1000);

  console.log(
    `${savedCount}개의 완료된 캔들을 Supabase에 반영했습니다.`,
  );

  console.log(
    "BTCUSDT 기술지표 계산을 시작합니다.",
  );
  await analyzeBtcTechnical();

  console.log(
    "BTCUSDT 시장점수 계산을 시작합니다.",
  );

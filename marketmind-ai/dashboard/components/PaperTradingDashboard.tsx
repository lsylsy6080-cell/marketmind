...
function conditionState(decision: FinalMarketDecision | undefined, config: PaperTradingData["config"]) {
  ...
  const isShort = decision.direction === "bearish" && decision.action === "sell";
  return [
    { label: "최종 점수", value: decision.final_score, target: isShort ? `≤ ${config.short_score_max}` : `≥ ${config.long_score_min}`, ok: isShort ? decision.final_score <= config.short_score_max : decision.final_score >= config.long_score_min },
    { label: "신뢰도", value: decision.final_confidence, target: `≥ ${config.confidence_min}`, ok: decision.final_confidence >= config.confidence_min },
...
export function PaperTradingDashboard({ data }: { data: PaperTradingData }) {
  ...
  const latestDecision = latestRun?.decision_id ? data.decisionsById[latestRun.decision_id] : undefined;
  ...
  return <section className="paper-dashboard">
    <header className="paper-title"><div><h1>모의매매 대시보드 <span>V1.2</span></h1><p>AI 기반 모의매매 현황과 실제 유지 중인 포지션을 확인합니다.</p></div><div className="paper-live"><i/>시스템 정상 · 최근 실행 {formatRelativeTime(runs[0]?.created_at ?? null)}</div></header>
    ...
    <article className="panel paper-summary"><h2>전략별 성과 요약 <small>{config?.strategy_version ?? "paper-trading-v1"}</small></h2><div><span>총 거래 수<b>{trades.length}</b></span><span>승리 거래 수<b>{wins}</b></span><span>패배 거래 수<b>{losses}</b></span><span>승률<b>{formatNumber(winRate,2)}%</b></span><span>평균 수익률<b className={avgReturn>=0?"paper-positive":"paper-negative"}>{formatPercent(avgReturn)}</b></span><span>수익 팩터<b>{formatNumber(profitFactor,2)}</b></span><span>누적 수수료<b>{formatNumber(account.total_fees,2)}</b></span><span>평균 보유 시간<b>{trades.length ? durationLabel(trades[Math.floor(trades.length/2)]?.opened_at ?? new Date().toISOString()) : "—"}</b></span></div></article>
  </section>;
}

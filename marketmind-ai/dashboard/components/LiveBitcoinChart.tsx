...
type ZoneGrade = "S" | "A" | "B" | "C";
type ImportantZone = {price:number;zoneLow:number;zoneHigh:number;strength:number;grade:ZoneGrade;status:string;distancePercent:number;kind:"support"|"resistance";timeframes:string[];touchCount:number;rejectionPercent:number;roleFlipCount:number;reasons:string[]};
type MarketStructureSnapshot = {calculatedAt:string;currentPrice:number;supportLevels:ImportantZone[];resistanceLevels:ImportantZone[];strategyVersion:string};
...
function selectImportant(rows:ImportantZone[]){const active=rows.filter(x=>x.status==="active");const top=active.filter(x=>x.grade==="S"||x.grade==="A");return (top.length?top:active.filter(x=>x.grade==="B")).slice(0,2)}
...
export function LiveBitcoinChart({positions=[],chartOnly=false,initialInterval="1d",chartHeight=640,title="BTCUSDT · 장기 추세 분석",simple=false}:{positions?:PaperPosition[];chartOnly?:boolean;initialInterval?:Interval;chartHeight?:number;title?:string;simple?:boolean}){
 ...
 const entryRef=useRef<IPriceLine|null>(null),stopRef=useRef<IPriceLine|null>(null),takeRef=useRef<IPriceLine|null>(null),supportRef=useRef<IPriceLine|null>(null),resistanceRef=useRef<IPriceLine|null>(null),importantZoneRefs=useRef<IPriceLine[]>([]);const markerApiRef=useRef<ISeriesMarkersPluginApi<Time>|null>(null);
 const candlesRef=useRef<Candle[]>([]),loadingOlderRef=useRef(false),hasMoreRef=useRef(true),intervalRef=useRef<Interval>(initialInterval),lastUiRefreshRef=useRef(0);
 const chartDisposedRef=useRef(true),cloudRafRef=useRef<number|null>(null);
 const [interval,setIntervalValue]=useState<Interval>(initialInterval),[candles,setCandles]=useState<Candle[]>([]),[status,setStatus]=useState<"loading"|"live"|"reconnecting"|"error">("loading"),[error,setError]=useState<string|null>(null),[loadingOlder,setLoadingOlder]=useState(false);
 const [overlays,setOverlays]=useState<Record<OverlayKey,boolean>>(simple?{ema:true,ichimoku:false,structure:false,sr:false,volume:true}:{ema:false,ichimoku:true,structure:true,sr:true,volume:true});
 const [viewMode,setViewMode]=useState<ViewMode>(simple?"full":"trend");
 ...
 const importantSupports=useMemo(()=>selectImportant(marketStructure?.supportLevels??[]),[marketStructure]);
 const importantResistances=useMemo(()=>selectImportant(marketStructure?.resistanceLevels??[]),[marketStructure]);
 ...
 useEffect(()=>{let dead=false,timer:ReturnType<typeof setInterval>|null=null;const load=async()=>{try{const r=await fetch("/api/market-structure",{cache:"no-store"}),p=await r.json();if(!dead&&p.ok&&p.current)setMarketStructure(p.current)}catch(e){console.warn("[매물대] 중요도 데이터 로드 실패",e)}};void load();timer=setInterval(load,60_000);return()=>{dead=true;if(timer)clearInterval(timer)}},[]);
 ...
 useEffect(()=>{
  ...
  return()=>{
   ...
   entryRef.current=null;stopRef.current=null;takeRef.current=null;supportRef.current=null;resistanceRef.current=null;importantZoneRefs.current=[];
 ...
 useEffect(()=>{
  ...
  try{
   ...
   for(const line of importantZoneRefs.current)candle.removePriceLine(line);importantZoneRefs.current=[];
  const important=[...importantSupports,...importantResistances];
  if(overlays.sr&&important.length){
   importantZoneRefs.current=important.map(z=>candle.createPriceLine({price:z.price,color:z.kind==='support'?(z.grade==='S'?"#10b981":"#4ade80"):(z.grade==='S'?"#fb7185":"#f59e0b"),lineWidth:z.grade==='S'?3:2,lineStyle:z.grade==='S'?LineStyle.Solid:LineStyle.Dashed,axisLabelVisible:true,title:`${z.grade}급 ${z.kind==='support'?'지지':'저항'} ${z.strength}`}));
  }else{
 ...
 },[selectedTrend,overlays.sr,overlays.structure,importantSupports,importantResistances]);
 ...
 return <section className={`panel live-chart-panel tradingview-chart-panel long-term-trend-panel ${chartOnly?"chart-only":""}`}>
  ...
  {marketStructure&&(importantSupports.length||importantResistances.length)?<div className="important-zone-board"><div><strong>중요 매물대</strong><small>S·A급 우선 · 60초 자동 갱신</small></div><section>{[...importantSupports,...importantResistances].map(z=><article key={`${z.kind}-${z.price}`} className={`${z.kind} grade-${z.grade.toLowerCase()}`}><header><b>{z.grade}급 {z.kind==='support'?'지지':'저항'}</b><em>{z.strength}/100</em></header><strong>{money(z.zoneLow)} ~ {money(z.zoneHigh)}</strong><p>{z.reasons.slice(0,2).join(" · ")}</p><small>{z.touchCount}회 반응 · 거절 {z.rejectionPercent.toFixed(2)}% · {z.timeframes.join("/")||"단일 시간봉"}</small></article>)}</section></div>:null}
